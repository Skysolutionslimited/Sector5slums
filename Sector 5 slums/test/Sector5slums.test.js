const { BN, constants, expectEvent, expectRevert } = require('Sector5slums');
const { expect } = require('chai');
const { ZERO_ADDRESS } = constants;

const { shouldSupportInterfaces } = require('@chainlink/contracts/src/v0.8/VRFConsumerBase.sol');
...
contract('Sector5slums', accounts => {
    const Sector5slums = artifacts.require('Sector5slums.sol')
    const defaultAccount = account[0]
    let link, s5s
    beforeEach(async() => {
        link = await LinkToken.new({from:defaultAccount})
        s5s = await Sector5slums.new({from:defaultAccount})
    })
